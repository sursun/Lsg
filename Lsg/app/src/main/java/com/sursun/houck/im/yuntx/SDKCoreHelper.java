...
import com.sursun.houck.common.HKPreferenceSettings;
import com.sursun.houck.common.HKPreferences;
import com.sursun.houck.common.LogUtil;
import com.sursun.houck.common.ToastUtil;
import com.sursun.houck.im.yuntx.dialog.ECProgressDialog;
import com.sursun.houck.lsg.LoginActivity;
...
//        ECProgressDialog mPostingdialog = new ECProgressDialog(mContext, R.string.login_posting);
//        mPostingdialog.show();

        ClientUser clientUser = new ClientUser(userId);

        CCPAppManager.setClientUser(clientUser);
        SDKCoreHelper.init(LsgApplication.getInstance() , ECInitParams.LoginMode.FORCE_LOGIN);
    }
...
//    }
    ...
    public static void init(Context ctx , ECInitParams.LoginMode mode) {
        ...
        ctx = LsgApplication.getInstance().getApplicationContext();
...
//    }
    ...
    public void onConnectState(ECDevice.ECConnectState state, ECError error) {
        if(state == ECDevice.ECConnectState.CONNECT_FAILED && error.errorCode == SdkErrorCode.SDK_KICKED_OFF) {
            try {
                HKPreferences.savePreference(HKPreferenceSettings.SETTINGS_REGIST_AUTO, "", true);
    ...
    private static void postConnectNotify() {
        if(getInstance().mContext instanceof LoginActivity) {
            ((LoginActivity) getInstance().mContext).onNetWorkNotify(getConnectState());
        }
    }
